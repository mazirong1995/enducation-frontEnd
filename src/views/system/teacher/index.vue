    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item v-if="form.userId == undefined" label="教师密码" prop="password">
              <el-input v-model="form.password" placeholder="请输入教师密码" type="password" maxlength="20" show-password/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
...
<script>
import { listUser,listTeacher,addUserTeacher, getUser, delUser, addUser, updateUser, resetUserPwd, changeUserStatus, deptTreeSelect } from "@/api/system/user";
...
export default {
  ...
  data() {
    return {
      ...
      // 列信息
      columns: [
        { key: 0, label: `教师编号`, visible: true },
        { key: 1, label: `教师名称`, visible: true },
        { key: 2, label: `教师昵称`, visible: true },
      ...
      rules: {
        ...
        password: [
          ...
          { min: 5, max: 20, message: '教师密码长度必须介于 5 和 20 之间', trigger: 'blur' }
        ],
        email: [
          {
            type: "email",
  ...
  methods: {
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要"' + text + '""' + row.userName + '"教师吗？').then(function() {
    ...
    handleResetPwd(row) {
      this.$prompt('请输入"' + row.userName + '"的新密码', "提示", {
        ...
        inputPattern: /^.{5,20}$/,
        inputErrorMessage: "教师密码长度必须介于 5 和 20 之间"
      }).then(({ value }) => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.userId != undefined) {
            ...
            addUserTeacher(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除所选教师的数据？').then(function() {
